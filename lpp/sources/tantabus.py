...
class Tantabus(TagSourceBase):
    def __init__(self, work_dir: str = "."):
        TagSourceBase.__init__(self,
                               "https://tantabus.ai/pages/search_syntax",
                               "Tantabus query or image URL",
                               work_dir)
        self.__api_key = None
    ...
    @attach_query_param("filter_type", "Tantabus Filter")
    ...
    def request_tags(self,
        ...
        ENDPOINT = "https://tantabus.ai/api/v1/json/search/images"
        PER_PAGE_MAX = 50
        QUERY_DELAY = 0.5

        image_id = re.search(
            r"^(?:https?:\/\/)?(?:tantabus\.ai\/images\/)?(\d+)(\?.*)?$", query
        )
        if image_id:
    ...
    def __fetch_user_filters(self) -> None:
        self._logger.info("Attempting to fetch Tantabus user filters...")
        try:
            json_response = self._send_api_request(
                "https://tantabus.ai/api/v1/json/filters/user",
                {"key": self.__api_key}
            )

            for filter in json_response["filters"]:
                self.__filter_ids[filter["name"]] = filter["id"]
            self._logger.info("Successfully fetched Tantabus user filters.")
        except (HTTPError, ConnectionError, TooManyRedirects):
            self._logger.warning(
                "Failed to fetch Tantabus user filters due to connection issues."
            )
        except Timeout:
            self._logger.warning(
                "Failed to fetch Tantabus filters due to connection timeout."
            )
        except Exception as e:
            self._logger.debug(
                f"Failed to fetch Tantabus user filters ({e:=})"
            )
